...
def get_data_augmentations():
    ...
    online_transforms = transforms.Compose([ 
        ...
        transforms.RandomResizedCrop(config.tile_size, scale=(0.2, 1.0)),
...
def train_model(model, data_loader, device, num_epochs, optimizer):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            images, folders, subfolders = batch_data
            ...
            # Update progress bar with current loss and folder info
            progress_bar.set_postfix({
                'loss': f'{loss.item():.4f}',
                'folder': folders[0],  # Show first folder in batch
                'subfolder': subfolders[0]  # Show first subfolder in batch
            })
...
def main():
    ...
    dataset_train = dataset.TileDatasetTrain(
        root_folder=config.folder_path_train,
        subfolder_names=config.subfolder_names,
